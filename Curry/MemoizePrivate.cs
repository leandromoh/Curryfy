...
namespace Functional
{
    public static partial class Prelude
    {
        ...
        private static string generateMemoize(int args)
        {
            var n = Enumerable.Range(1, args).ToArray();

            return string.Format(
@"
        public static Func<{0}, TResult> Memoize<{0}, TResult>(this Func<{0}, TResult> function, int? cacheExpirationInSeconds)
        {{
            var y = new {{ {1} }};
            var dic = Enumerable.Repeat(default(TResult), 0).ToDictionary(x => y);
            var expirationDate = cacheExpirationInSeconds.HasValue 
